...
func (w *FileWriter) Write(p []byte) (n int, err error) {
	if w.Rotate {
		...
		when := time.Now()
		d := when.Day()
		if w.needRotate(len(p), d) {
			w.RUnlock()
			w.Lock()
			if w.needRotate(len(p), d) {
				if rerr := w.doRotate(when); rerr != nil {
					fmt.Fprintf(os.Stderr, "Log FileWriter(%q): %s\n", w.Filename, rerr)
				}
			}
			w.Unlock()
		} else {
	...
	w.Lock()
	if n, err = w.fileWriter.Write(p); err == nil {
		w.maxLinesCurLines++
		w.maxSizeCurSize += len(p)
	}
	w.Unlock()
	return n, err
}
